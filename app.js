...
function render() {
  ...
  papers = papers.filter(p => (state.topic === "all" || p.topics.includes(state.topic)) && (state.journal === "all" || p.journal === state.journal) && (!state.search || [p.title,p.analysis?.title_zh,p.analysis?.summary_zh,p.journal,p.doi].join(" ").toLowerCase().includes(state.search)));
